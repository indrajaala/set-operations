...
test('subset and superset', () => {
    ...
    expect(isSubSet(new Set([4, 5]), new Set([1, 9, 4, 8, 34, 43, 5]))).toBe(true);
    expect(isSubSet(new Set([4, 5, 6]), new Set([1, 9, 4, 8, 34, 43, 5]))).toBe(false);
...
test('union', () => {
    ...
    expect(union(new Set([1,3]), new Set([2]))).toEqual(new Set([1, 2, 3]));
